function [angle, error_angle] = interpolation(estimation_points, calib_points, folder_root, person_data, program_options)
...
elseif( program_options.method_interp == 3) %% poly = 10
    V = XLcal;
    P = [TargetPosCalXcal TargetPosCalYcal];
    % PV = @(b,V)  [[(b(1).*V(:,1)+b(2))] [(b(3).*V(:,2)+b(4))]]; % Objective Function: [b2 + b1*x] [b4 + b3*y]
    PV = @(b,V) [ ...
        (  b(10)*(V(:,1).^2).*V(:,2) +  b(9).*(V(:,2).^2).*V(:,1) +  b(8).*V(:,1).^3 +  b(7).*V(:,2).^3 +  b(6)*V(:,1).*V(:,2) +  b(5).*V(:,2).^2 +   b(4).*V(:,1).^2 +  b(3).*V(:,2) +  b(2).*V(:,1) +  b(1) ) ...
        (  b(20)*(V(:,2).^2).*V(:,1) + b(19).*(V(:,1).^2).*V(:,2) + b(18).*V(:,2).^3 + b(17).*V(:,1).^3 + b(16)*V(:,2).*V(:,1) + b(15).*V(:,1).^2 +  b(14).*V(:,2).^2 + b(13).*V(:,1) + b(12).*V(:,2) + b(11) )  ];
    [B, rn, r] = lsqcurvefit(PV, ones(20,1), V, P);
    Pc = PV(B,XL);
    plx = Pc(:,1);
    ply = Pc(:,2);
    ...
    prx = Pc(:,1);
    pry = Pc(:,2);
elseif( program_options.method_interp == 4) %% poly = 3
    V = XLcal;
    P = [TargetPosCalXcal TargetPosCalYcal];
    % PV = @(b,V)  [[(b(1).*V(:,1)+b(2))] [(b(3).*V(:,2)+b(4))]]; % Objective Function: [b2 + b1*x] [b4 + b3*y]
    PV = @(b,V) [ ...
        (  b(3).*V(:,2) + b(2).*V(:,1) + b(1) ) ...
        (  b(6).*V(:,1) + b(5).*V(:,2) + b(4) )  ];
    [B, rn, r] = lsqcurvefit(PV, ones(6,1), V, P);
    Pc = PV(B,XL);
    plx = Pc(:,1);
    ply = Pc(:,2);
